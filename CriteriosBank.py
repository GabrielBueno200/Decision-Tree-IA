...
def criteriosBank():
    ...
    attributesValues = (np.asarray(data[attr]).reshape(-1, 1) for attr in attributes)
    ...
    tree.plot_tree(Arvore, feature_names=attributes, class_names=['yes', 'no'], filled=True, rounded=True)
    plt.show()
